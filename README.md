...
- `src/lib/site.ts`: Public Discord invite, application form, and contact links. The supplied Discord invite is authoritative.
...
Current SVGs are in `public/brand/`. The HACK badge and infinity motif preserve the existing colors and outlined wordmark. Motion follows the figure-eight with a tapered trail; reduced-motion users receive the static version. The favicon uses the static infinity mark.
