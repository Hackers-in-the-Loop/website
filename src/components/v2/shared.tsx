...
export function FounderLinks() {
  return <div className="v2-actions"><a className="v2-button" href={siteConfig.linkedinUrl} target="_blank" rel="noreferrer">LinkedIn ↗</a><a className="v2-link" href={siteConfig.xUrl} target="_blank" rel="noreferrer">X ↗</a></div>;
}

export function PageIntro({ label, title, children }: { label: string; title: string; children: React.ReactNode }) {
  return <header className="v2-hero-surface"><div className="v2-wrap v2-page-intro"><p className="v2-kicker">{label}</p><h1>{title}</h1><div className="v2-intro-copy">{children}</div></div></header>;
}
